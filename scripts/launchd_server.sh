...
LABEL="${LABEL:-com.okash1n.fwai.server}"
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
PLIST_DIR="$HOME/Library/LaunchAgents"
PLIST_PATH="$PLIST_DIR/$LABEL.plist"
LOG_DIR="$ROOT_DIR/logs"
STDOUT_LOG="$LOG_DIR/server.launchd.out.log"
STDERR_LOG="$LOG_DIR/server.launchd.err.log"
DOMAIN="gui/$(id -u)"
...
<dict>
  ...
  <array>
    <string>$ROOT_DIR/scripts/start_server.sh</string>
  </array>
...
status_service() {
  ...
  echo "ports:"
  lsof -nP -iUDP:34197 || true
  lsof -nP -iTCP:27015 -sTCP:LISTEN || true
}
...
usage() {
  cat <<'EOF'
Usage: ./scripts/launchd_server.sh <command>

Commands:
  install     Write plist and bootstrap LaunchAgent
  uninstall   Stop and remove LaunchAgent plist
  start       Start server via launchd (installs if missing)
  stop        Stop running service
  restart     Stop then start
  status      Show launchd state and listening ports
