...
- board や Kanban 更新の依頼では、まず `.project/skills/fwai-board-maintainer/SKILL.md` を読む
- 実装進捗を書く前に、必ず現状のコード・設定・起動状態を確認する
- ローカルの進捗管理は `board/cards/*.md` と `board/board.json` を優先し、必要に応じて更新する
- 実装や方針の状態が変わったら、関連する `board/cards/*.md` を更新する
- 完了したタスクは `done` に移し、残作業は別カードへ切り出す
...
- 記事の見出しレベルは `###` までに制限する
- note 用記事ではインラインコードを使わない。インラインコードの代替として丸括弧を使わず、技術用語は太字か『』か [ ] で表記する。丸括弧が自然な文脈では使ってよい
- 記事冒頭の固定構成は「リード文 → `## 固定リンク` → `## 今回の進捗`」とする
- リード文は短くまとめ、今回やったことと到達点を先に言う
- 冒頭で経緯説明を長く引っ張らず、同じ内容を言い換えて繰り返さない
- `## 固定リンク` には少なくともリポジトリと記事まとめを載せる
